...
<template>
  <section aria-labelledby="wp-titel">
    ...
    <Dialog v-model:visible="nieuwOpen" modal :closable="false" header="Nieuw werkpakket" data-testid="wp-nieuw-dialog">
      <form class="flex flex-col gap-[var(--cd-space-md)] min-w-[24rem]" data-testid="wp-nieuw-form" @submit.prevent="bevestigNieuw">
        <div class="flex flex-col gap-[var(--cd-space-xs)]">
          <label for="wp-naam" class="font-semibold">Naam *</label>
          <InputText id="wp-naam" v-model="form.naam" data-testid="wp-naam" :aria-invalid="!!fouten.naam" />
          <span v-if="fouten.naam" role="alert" data-testid="wp-fout-naam" class="text-[var(--cd-color-danger)] text-[length:var(--cd-text-sm)]">{{ fouten.naam }}</span>
        </div>
        <div class="flex flex-col gap-[var(--cd-space-xs)]">
