...
De applicatie verbindt met de database als `lk_app` — een non-superuser die
onderworpen is aan Row Level Security (RLS). Uit de eerste live DB-run (P4)
bleek concreet:

- `lk_app` heeft **geen `CREATE` op schema `public`**
  (`has_schema_privilege('lk_app','public','CREATE') = false`); `lk_app` kan
  dus geen tabellen/enums aanmaken en kan `alembic upgrade head` niet draaien.
- `lk_admin` is superuser/owner en voert de DDL uit; door
...
`lk-api` start pas na succesvolle afronding van de init-container:

```yaml
depends_on:
  postgres: { condition: service_healthy }
  lk-migrate: { condition: service_completed_successfully }
```
...
- `lk_admin` komt **uitsluitend** in de init-container voor.
- De **app-container houdt uitsluitend `lk_app`**; geen admin-credential in
...
- **Operationeel**: één deterministische volgorde — DB healthy → migrate+seed
  (lk_admin) → app (lk_app). Een verse `docker compose up` levert een volledig
...
- **Security**: het admin-credential heeft een minimaal blootstellingsvlak
  ...
  rechtenarm als `lk_app` onder RLS.
...
- **Lokaal/CI alternatief**: dezelfde twee commando's kunnen handmatig
  (`alembic upgrade head` als lk_admin, daarna `python3 -m app.platform_init`)
...
- **App-entrypoint migreert zelf** (`alembic` in de api-entrypoint):
  verworpen — de app-container draait als `lk_app` zonder `CREATE`, dus de
...
- **App-container met twee credentials** (lk_admin voor migratie, lk_app voor
  runtime): verworpen — `lk_admin` in de app-laag is expliciet verboden
